(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            showth: true, //是否显示表头
            height: '240', //flexigrid插件的高度，单位为px
            width: 'auto', //宽度值，auto表示根据每列的宽度自动计算
            striped: false, //是否显示斑纹效果，默认是奇偶交互的形式
            novstripe: false, //是否显示列交替色，暂时有bug
            minwidth: 20, //列的最小宽度
            minheight: 80, //列的最小高度
            ...
            title: false, //是否包含标题
            pagestat: '显示记录从{from}到{to}{offset}，总数 {total} 条', //显示当前页和总页面的样式
        ...
        var g = {
            ...
            rePosDrag: function() {
                ...
                $(g.cDrag).css({ top: $(g.hDiv).position().top + 1, left: $(g.hDiv).position().left });
                var cdpad = this.cdpad;

                $('div', g.cDrag).hide();
                $('thead tr:first th', this.hDiv).each(function(i) {
                    if ($(this).css("display") == "none") {
                        return true;
                    }
                    var cdpos = parseInt($('div', this).width());
                    cdpos = $('div', this).width() + $('div', this).position().left + Math.floor($('div:eq(' + i + ')', g.cDrag).width()/2);
                    $('div:eq(' + i + ')', g.cDrag).css({ 'left': cdpos + 'px'}).show();
                }
				);
            ...
            dragStart: function(dragtype, e, obj) { //default drag function start
                ...
                if (dragtype == 'colresize') //column resize
                {
                    ...
                    var ow = $('th:eq(' + n + ') div', this.hDiv).width();
                    $(obj).addClass('dragging').siblings().hide();
                    /*
                    var prev=$(obj).prev();
                    if(prev)
                        $(prev).addClass('dragging').show();
                    */
            ...
            dragMove: function(e) {
                ...
                else if (this.vresize) //table resize
                {
                    ...
                    $(this.colCopy).css({ top: e.pageY + 6, left: e.pageX + 8, display: 'block' });
                }
            ...
            dragEnd: function() {
                if (this.colresize) {
                    ...
                    $('th:eq(' + n + ') div', this.hDiv).css('width', nw);

                    $('tr', this.bDiv).each(
									function() {
									    //$('td:visible div:eq(' + n + ')', this).css('width', nw);
									    //$('td:visible:eq(' + n + ') div', this).css('width', nw);
									    $('td:eq(' + n + ') div', this).css('width', nw);
									}
								);
                    ...
                    if (this.dcolt != null) {
                        ...
                        var $cF = this.rePosDrag;
                        setTimeout(function(){$cF.call();},100);
                    }
                    this.dcol = null;
                    this.hset = null;
                    this.dcoln = null;
            ...
            removeRowFromDic: function(rowid) {
                if (dic.exists(rowid)) {
                    dic.remove(rowid);
                }
            },
            ...
            buildpager: function() { //rebuild pager based on new properties
                if (p.usepager) {
                    ...
                    //增加js增加删除的变更提示
                    var offset = (p.rpoffset == 0) ? '' :
                        ((p.rpoffset > 0) ? "+" + p.rpoffset : p.rpoffset);
                    stat = stat.replace(/{offset}/, offset);
                    if (this.pDiv)
            ...
            doSearch: function() {
                ...
                if (p.query != "" && p.searchitems && index >= 0 && p.searchitems.length > index) {
                    if (p.searchitems[index].reg) {
                        var isV = false;
                        try {
                            isV = p.searchitems[index].reg.test(p.query);
                        } catch (ee) {
                            alert("你输入的正则表达式可能有误，错误信息为:" + ee + "，正则为" + p.searchitems[index].reg);
                        }
                        if (!isV) {
                            alert("你的输入不符合要求!");
                            return;
            ...
            getSelectedRowsIds: function() {
                ...
                $("tr.trSelected", g.bDiv).each(function() {
                    ids.push($(this).attr('id').substr(3));
                });
                return ids;
            },
            ...
            getGridJsonData: function() {
                ...
                for (var i = 0; i < kl.length; i++) {
                    r += '{id: \'' + kl[i] + '\',\r\n';
                    r += 'cell: [\r\n';
                    var item = dic.item(kl[i]);
                    var vl = item.keys();
                    for (var j = 0; j < vl.length; j++) {
                        var v = base64encode(utf16to8(item.item(vl[j])));
                        r += '\'' + v + '\',';
                    }
            ...
            //判断指定Id的行是否存在
            containsRowId: function(rowid) {
                if (rowid) {
                    return dic.exists(rowid);
                }
                else
                    return false;
            },
            ...
            updateRowData: function(row) {
                var self = this;
                if (row && row.cell && row.id) {
                    var ths = $('thead tr:first th', g.hDiv);
                    var escid = self.jQueryEscape(row.id);
                    var tr = $("#row" + escid, g.bDiv).eq(0);
                    var showc = p.showcheckbox;
                    if (dic.exists(row.id)) {
                        var dicValue = new Dictionary();
                        var dicValueKey, dicValueValue;
                        $(ths).each(function(i) {
                            var idx = $(this).attr('axis').substr(3);
                            ...
                            if (td) {
                                if (idx >= 0 && row.cell[idx]) {
                                    ...
                                    td.innerHTML = divHtml;
                                }
                            }
                        });
                        dic.remove(row.id);
                        dic.add(row.id, dicValue);
                    }
                }
            },
            ...
            initCheckProp: function() {
                //处理初始化选中
                var $tr = $(this);
                var ths = $('thead tr:first th', g.hDiv);
                var index = -1;
                $(ths).each(function(i) {
                    if ($(this).attr('checkCol'))
                        index = i; return; //找到第一个跳出循环
                });
                if (index >= 0) {
                    var checked = $('td', $tr).eq(index).text().toLowerCase();
                    if (checked == 'true') {
                        if (p.showcheckbox) {
            ...
            checkhandler: function() {
                ...
                //修复点击bug
                if (p.showcheckbox && p.onrowchecked && $ck.length > 0) {
                    p.onrowchecked.call($ck[0]);
                }
            },
            addRowProp: function() {
                var $gF = this.rowProp;
                var $kF = this.initCheckProp;
                        ...
                        if (p.showcheckbox) {
                            $("input.itemchk", this).each(function() {
                                $(this).click(function(e) {
                                    ...
                                    $cf.call(ptr[0]);
                                    /*
                                    if (p.onrowchecked&&!p.selectedonclick) {
                                    p.onrowchecked.call(this);
                                    }
                                    */
                                    e.stopPropagation();
            ...
            checkAllOrNot: function(parent) {
                ...
                $("input.itemchk", g.bDiv).each(function() {
                    ...
                    //Raise Event
                    if (p.onrowchecked && old != this.checked) {
                        p.onrowchecked.call(this);
                    }
                });
            },
        ...
        if (p.colModel) {
            ...
            for (i = 0; i < p.colModel.length; i++) {
                ...
                //checkField列,为th增加一个属性checkCol
                if (cm.checkField)
                    $(th).attr('checkCol', true);
                if (cm.name)
			 	    ...
			 	    if (!$(this).attr("isch")) {
						...
						.hover(
							function() {
							    ...
							    if ($(this).attr('abbr') != p.sortname && !g.colCopy && !g.colresize && $(this).attr('sort'))
							        $('div', this).addClass('s' + p.sortorder);
							    else if ($(this).attr('abbr') == p.sortname && !g.colCopy && !g.colresize && $(this).attr('sort')) {
							        var no = '';
							    ...
							    if (g.colCopy) {
							        ...
							        var n = $('th', g.hDiv).index(this);
							        var onl = parseInt($(this).position().left) + $(this).width();
							        ...
							        $('div', g.cDrag).removeClass('dragging');
							        $('div:eq(' + n + ')', g.cDrag).addClass('dragging');
        ...
        if (cdcol != null) {
            ...
            //alert(g.cdpad);
            if (g.cdpad == 0) {
                g.cdpad = 6; //跟下面情况一样，不知什么情况下取不到对应的值。故设置为6
            }
            $(g.bDiv).before(g.cDrag);

            var cdheight = $(g.bDiv).height();
            var hdheight = $(g.hDiv).height();
            $('thead tr:first th', g.hDiv).each//为每一个表头创建了一个宽度调整div
			(
			 	function() {
			 	    var cgDiv = document.createElement('div');
			 	    $(g.cDrag).append(cgDiv);
			 	    if (!p.cgwidth) p.cgwidth = 6; //$(cgDiv).width();//modify by hua，特殊情况下取的宽度很奇怪。故设置为4
        ...
        if (p.usepager) {
            ...
            if (p.searchitems && p.searchitems.length > 0) {
                ...
                $('input[name=q],select[name=qtype]', g.sDiv).keydown(function(e) { if (e.keyCode == 13) { g.doSearch(); return false; } });
    ...
    $.fn.updateRowData = function(row) {
        if (this[0].grid) {
            this[0].grid.updateRowData(row);
        }
    };
    //获取指定id行的数据，数组格式
